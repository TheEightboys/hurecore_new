...
import {
    ...
    scheduleBlocksAPI, payrollAPI, verificationAPI, auditAPI, documentsAPI, getClinicId
} from './employerApi';
...
export default function EmployerDashboard() {
    ...
    const DocsView = () => {
        ...
        const [uploadForm, setUploadForm] = useState({ name: '', category: 'other', file: null });
        ...
        const loadDocuments = async () => {
            try {
                setLoading(true);
                const result = await documentsAPI.list(clinicId, categoryFilter);
        ...
        const handleUpload = async () => {
            ...
            try {
                setUploading(true);
                await documentsAPI.upload(clinicId, uploadForm.file, {
                    name: uploadForm.name || uploadForm.file.name,
                    category: uploadForm.category,
                    uploadedByName: org?.name || 'Admin'
                });
        ...
        const handleDownload = async (doc) => {
            try {
                const result = await documentsAPI.getDownloadUrl(clinicId, doc.id);
        ...
        const handleDelete = async (doc) => {
            ...
            try {
                await documentsAPI.delete(clinicId, doc.id);
                loadDocuments();
            } catch (err) {
                            ...
                            <tbody>
                                {documents.map(doc => (
                                    <tr key={doc.id} className="border-t hover:bg-slate-50">
                                        ...
                                        <td className="p-3">
                                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getCategoryBadge(doc.category)}`}>
                                                {doc.category || 'other'}
                                            </span>
                                        </td>
                                        <td className="p-3 text-slate-600">{doc.uploaded_by_name || 'Unknown'}</td>
                                        <td className="p-3 text-slate-600">
                                            {doc.created_at ? new Date(doc.created_at).toLocaleDateString() : '-'}
                                        </td>
                                        <td className="p-3 text-slate-600">{formatFileSize(doc.file_size)}</td>
                                        <td className="p-3 text-right">
                                            <button
                                                onClick={() => handleDownload(doc)}
                                                className="px-3 py-1 text-emerald-600 hover:bg-emerald-50 rounded text-sm mr-2"
                                            >
                                                Download
                                            </button>
                                            <button
                                                onClick={() => handleDelete(doc)}
                                                className="px-3 py-1 text-rose-600 hover:bg-rose-50 rounded text-sm"
                                            >
                                                Delete
                                            </button>
                                        </td>
